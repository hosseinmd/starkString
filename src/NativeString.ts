class NativeString {
   ...
   *
   * @param {number} index The zero-based index of the desired character. If there
   ...
   * element at position pos in the String resulting from converting this object to
   * a String. If there is no element at that position, the result is undefined. If
   * a valid UTF-16 surrogate pair does not begin at pos, the result is the code
   * unit at pos.
   *
   * @param {number} pos
   */
   ...
   * is the same as the corresponding elements of this object (converted to a
   * String) starting at endPosition – length(this). Otherwise returns false.
   *
   ...
   * this object to a String, at one or more positions that are greater than or
   * equal to position; otherwise, returns false.
   *
   ...
   * @param {number} [position] The index at which to begin searching. If omitted,
  ...
  /**
   * Matches a string an object that supports being matched against, and returns an
   * array containing the results of that search.
   *
   * @param {{ [Symbol.match](string: string): RegExpMatchArray | null }} matcher An
   *     object that supports being matched against.
   ...
   *
   * @param {"NFC" | "NFD" | "NFKC" | "NFKD"} form Applicable values: "NFC", "NFD",
   *     "NFKC", or "NFKD", If not specified default is "NFC"
   */
   ...
   *
   * @param {number} maxLength The length of the resulting string once the current
   ...
   * @param {string} [fillString] The string to pad the current string with. If this
   ...
   * @param {string} replaceValue A string containing the text to replace for every
   ...
   *
   * @param {string | RegExp} regexp The regular expression pattern and applicable
   *     flags.
   * @returns {number}
   */
   ...
   *
   * @param {number} [start] The index to the beginning of the specified portion of
   *     stringObj.
   * @param {number} [end] The index to the end of the specified portion of
   *     stringObj. The substring includes the characters up to, but not including,
   *     the character indicated by end. If this value is not specified, the
   ...
   * @param {number} [limit] A value used to limit the number of elements returned
   *     in the array.
   * @returns {string[]}
   */
  split(
   ...
   * length.
   *
   * @param {number} from The starting position of the desired substring. The index
   *     of the first character in the string is zero.
   * @param {number} [length] The number of characters to include in the returned
   *     substring.
   */
   ...
   *
   * @param {number} start The zero-based index number indicating the beginning of
   *     the substring.
   * @param {number} [end] Zero-based index number indicating the end of the
   *     substring. The substring includes the characters up to, but not including,
   *     the character indicated by end. If end is omitted, the characters from
   ...
   * Returns true if the sequence of elements of searchString converted to a String
   * is the same as the corresponding elements of this object (converted to a
   * String) starting at position. Otherwise returns false.
   *
   * @param {string} searchString
